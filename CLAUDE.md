...
- **边语义**（`db.EDGE_TYPES` 4 种；节点类型 `db.NODE_TYPES` 现仅 concept，`add_edge` 按 `db.EDGE_ENDPOINT_TYPES` 矩阵校验端点节点类型，守卫 `bad_edge_endpoints` 兜底，新节点类型须在矩阵登记）：`related_to` 受限于 `db.RELATED_KINDS` 三种情形（同题替代/演化启发/教学对比），LLM 必须给 kind 字段，无效即丢弃，kind 存为 rationale 前缀；`prerequisite_of` 是唯一允许推断的边，推断边 rationale 加 `[推断]` 前缀且 confidence 封顶 `ingest.INFERRED_MAX_CONFIDENCE`（0.6）。
