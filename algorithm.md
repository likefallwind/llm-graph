...
圈定领域子语料（corpus crawl/grow：内链扩散，本地缓存+revision）
  → 结构挖掘（mine aliases/categories：别名/分类边候选/链接密度，零 LLM）
  → LLM 批量抽取（ingest --batch：只做判断——边类型/粒度/证据）
  → 去重 → proposed → 人工审核（review）→ 守卫（check）
```
...
- LLM 产出的一切节点和边都先进 `proposed`，人工审核后才生效。
- 所有图算法（守卫、先修链、度数、选点）只在 `seed + approved` 子图上运行，
  未审核的提议污染不了图的结构性质。
- `source` 字段全程溯源：`seed:...`、`mine:category:<lang>`、
...
2. 每个概念、每条边必须附正文原文摘录（≤60 字）作为 evidence
   （唯一例外：先修边允许推断级，见 §2）；
3. 粒度规则（§3）优先级最高；
4. related_to 必须带合法 kind（§2）；
5. 宁缺毋滥，每次最多提取 `--limit` 个概念。
...
### 6.5 审核（`kg/cli.py` review）

先审节点再审边（边要求两端节点均已生效才出现在队列里）：

- 节点：**a** 批准 / **r** 拒绝（级联拒绝其关联的 proposed 边）/
  **m** 合并到已有节点（名字变别名，边转移到目标节点，原节点 reject）/ **s** 跳过。
- 边：a / r / s。
- 审核结束自动跑一遍守卫。
