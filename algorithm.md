...
**误区（Misconception）是一类特殊 facet**（2026-07-06 新增）：正文明确指出的
常见错误理解/易混淆点，以 `误区:` 前缀（`db.MISCONCEPTION_PREFIX`）存进归属概念的
facets（如「误区:更深的网络一定效果更好」，陈述 ≤40 字短句而非名词短语）。
提取走 ingest 全部约束（§6.2：有据、evidence 机械校验、重引重试），归属概念
找不到就丢弃；教学导出时拆出为独立的 misconceptions 字段（§9）。
学习者层落地后再考虑升级为可挂诊断证据的独立类型。
...
7. **误区提取**（2026-07-06 新增，§3）：正文明确指出的常见错误理解/易混淆点
   （「常被误认为」「注意不要混淆」类表述）输出到 misconceptions
   （归属概念 + ≤40 字陈述 + evidence），同样过 evidence 机械校验与重引重试，
   归属概念（本次概念或图谱已有节点，含锚点）找不到就丢弃；
   与 anchor_facets 同理直接生效——有正文依据且不改拓扑，不占概念预算。
...
**影子模式（`kg verify --apply --dry-run`，burn-in 校准实验，2026-07-06 新增）**：
跑与真 `--apply` 完全相同的裁决逻辑（含分级把关、邻居重叠、环闸的邻接表模拟），
但不改任何状态、不写 review_log，只把「本会怎么裁」写进
`review_signals.signals.auto_would`（approve/reject）；影子批准的节点在本次运行内
视同生效，其关联边照常进入影子边裁决——与真跑的行为一致。之后条目照常走人工
审核（金标），`kg calibrate` 对比 auto_would 与人工首次裁决的一致率（§7.1），
一致率够高的通道才放开真 `--apply`。这是自动裁决上线前的标准动作：
先影子跑攒对比数据，再放行。
...
- 节点：**a** 批准 / **r** 拒绝（级联拒绝其关联的 proposed 边）/
  ...
  **节点批准后就地顺带裁决其关联边**（2026-07-06 新增）：另一端已生效的
  proposed 边立即弹出边裁决（与边阶段同一套 `_adjudicate_edge`，review_log
  照常逐条记录）——上下文还在审核者脑子里，不必等到边审核阶段重新建立；
...
- **burn-in 影子裁决一致率**（2026-07-06 新增）：`verify --apply --dry-run` 记的
  auto_would vs 此后人工首次裁决，按（通道 × 节点/边）分组——
  agree 同判 / disagree 反判（approve↔reject）/ modified 人工做了
  merge/flip/retype/demote（提议只对了一部分，影子批准算不完全对）/ pending 未裁；
  一致率 = agree/(agree+disagree+modified)，够高的通道才放开真 `--apply`。
...
`neighborhood(name)` 返回一个节点的教学上下文 JSON：定义、facets、
misconceptions（`误区:` 前缀 facet 拆出，去前缀——facet 是讲解侧面，
误区是要主动预防/诊断的错误认识，教学系统用法完全不同）、直接先修、
先修链（沿 prerequisite 反向 BFS 收集全部祖先）、下游解锁（unlocks）、
is_a/part_of 分类边、**resources 讲解资源**（2026-07-06 新增，零 LLM）：
该概念在哪些语料有讲解——各教材的对应章节（`docs.section_for_node`，含
sec_id/url/命中方式）+ 维基页（`corpus.page_for_node`），顺序即推荐优先级
（教材标题命中 > 维基页 > 教材正文提及）。未来个性化教学系统只依赖这个接口，
不直接碰库。
...
kg verify --limit 20       # 结构佐证（含 toc/邻居重叠）+ LLM 判断题复核
                           ...
                           #   --apply --dry-run 影子裁决（burn-in，只记不动）
kg review                  # 人工裁决剩余队列（节点批准后就地裁决关联边；
                           #   --audit N 抽检自动放行）
...
未做（记在 plan.md 里程碑）：学习者掌握度层（状态节点 + 只追加证据 +
行为信号不让 LLM 猜）+ 教学 prompt 接口；误区升级为可挂诊断证据的独立类型
（学习者层落地后）。burn-in 校准实验的**工具已就绪**（影子模式 + calibrate
一致率表），实验本身随人工金标裁决积累进行。
