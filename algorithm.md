...
source 记 `doc:...`，约束与 wiki 通道完全一致。`--batch` 暂不支持 `--doc`。
...
- 节点只自动批准：LLM 判「独立概念」+ 结构信号 `has_page` 且 `exact_title`
  （名字精确命中语料页标题/重定向）；拒绝与降级 facet 一律留人工
  （降级要人选归属概念）。先裁节点再裁边——节点生效后其关联边才能进入边裁决，
  刚生效节点的边通常还没有 LLM 结论，下一轮 verify 补上；
- 边批准：LLM「支持」且方向无疑 + 至少一项结构佐证（互链/Wikidata/toc=+1）
  + refd 与 toc 都不反对——推断先修边 + LLM「支持」+ 教材目录序一致即可自动批准；
...
kg ingest <锚点> --doc     # 教材通道提取（补教学性关系）
kg expand                  # 假设生成器补盲区（辅助）
kg verify --limit 20       # 结构佐证（含 toc）+ LLM 判断题复核（--apply 双重一致自动裁决）
kg review                  # 人工裁决剩余队列（--audit N 抽检自动放行）
kg calibrate               # 定期看各通道×佐证组合的 precision，调自动裁决松紧
kg check → kg viz
```

未做（记在 plan.md 里程碑）：教材锚点的缺口驱动选点（`--batch` × `--doc`）、
burn-in 质量校准实验（calibrate 已就绪，攒金标裁决后跑）、学习者掌握度层。
